...
function getValueForAdvancedSearch() {
    ...
    tabRecipes.forEach((recipe) => {
        ...
        if( !tabAppliances.find(element => recipe.appliance.toLowerCase() === element.toLowerCase())) {
            tabAppliances.push(recipe.appliance);
        }
        recipe.ustensils.forEach((ustensils) => {
            if ( !tabUstensils.find(element => ustensils.toLowerCase() === element.toLowerCase())) {
                tabUstensils.push(ustensils);
            }
        });
    });
    valueReturn.push(tabIngredients);
    valueReturn.push(tabAppliances);
    valueReturn.push(tabUstensils);
    return valueReturn;
}
...
btnCloseAppliancesSuggestion.addEventListener('click', function() { 
    closeSuggestions(btnCloseAppliancesSuggestion.parentElement.parentElement, suggestionAppliancesDOM);
});
btnCloseUstensilsSuggestion.addEventListener('click', function() { 
